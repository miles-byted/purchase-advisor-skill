...
├── SKILL.md                      # 九步半方法论主体（v3.2.0：含时效窗口 + 海内外型号映射）
├── references/
│   └── trusted-reviewers.md      # 可信测评站名录 + 独立性/行货判断表
└── examples/
    ├── robot-vacuum-walkthrough.md   # 中国本土强势品类（中文 70%）实录，含假洋牌四查斐纳案例
    └── air-purifier-walkthrough.md   # 欧美主导品类（英文 70%）实录，含海外型号→国行无对等的硬性拦截
```
...
- [空气净化器实录](skills/cross-lingual-purchase-advisor/examples/air-purifier-walkthrough.md)（欧美主导品类 · 英文 70%，含海外型号→国行无对等的第六步半硬拦截 + 泰拉蒙 Red Dot 主体实锤）
